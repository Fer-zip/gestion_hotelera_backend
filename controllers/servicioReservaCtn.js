...
exports.getAllServiciosReservas = (req,res)=>{
    conn.query('SELECT * from servicios_reserva',(err,rows)=>{
        if (err) throw err
        res.json(rows)
    })
}
...
exports.getUniqueServicioReserva = (req,res)=>{
    ...
    conn.query('SELECT * from servicios_reserva WHERE id_reserva = ?',[id_reserva],(err,rows)=>{
        if (err) throw err
        res.json(rows)
    })
}
...
exports.createServicioReserva = (req, res)=>{
    ...
    conn.query('INSERT INTO servicios_reserva (id_reserva,id_servicio,cantidad) VALUES (?,?,?)',[id_reserva,id_servicio,cantidad], (err,result)=>{
        if (err) throw err
...
exports.updateServicioReserva = (req, res)=>{
    ...
    conn.query('UPDATE servicios_reserva SET id_servicio = ?, cantidad = ? WHERE id = ?',[id_servicio,cantidad,id], (err,result)=>{
        if (err) throw err
