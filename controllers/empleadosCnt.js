...
exports.processLogin = (req, res)=>{
    ...
    conn.query(`SELECT * FROM empleados WHERE correo = ? AND contraseña= ?`, [correo_admin,pass_admin], (err,rows)=>{
        if (err) throw err
...
exports.createEmpleado = (req, res)=>{
    ...
    conn.query('INSERT INTO empleados (dni,nombre,apellido,cargo,telefono,correo,contraseña,fecha_contratacion) VALUES (?,?,?,?,?,?,?,?)',[dni,nombre,apellido,cargo,telefono,email,contra,fecha_contratacion], (err,result)=>{
        if (err) throw err
...
exports.updateEmpleado = (req, res)=>{
    ...
    conn.query('UPDATE empleados SET nombre = ?, apellido = ?, cargo = ?, telefono = ?, correo = ?, contraseña = ?, fecha_contratacion = ? WHERE dni = ?',[nombre,apellido,cargo,telefono,email,contra,fecha_contratacion,dni], (err,result)=>{
        if (err) throw err
...
exports.deleteEmpleado = (req, res)=>{
    const {dni} = req.params
    conn.query('DELETE from empleados WHERE dni = ?',[dni], (err,result)=>{
        if (err) throw err
...
exports.getEmpleado = (req, res)=>{
    const {dni} = req.params
    conn.query('SELECT * FROM empleados WHERE dni = ?',[dni], (err,rows)=>{
        if (err) throw err
        res.json(rows)
    })
}